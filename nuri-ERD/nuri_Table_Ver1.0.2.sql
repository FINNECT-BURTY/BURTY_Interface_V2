...
CREATE TABLE tbl_session (
                             session_id          CHAR(36)        NOT NULL COMMENT '세션 UUID',
                             user_id             BINARY(16)      NOT NULL,
                             device_id           BINARY(16)      NULL,
                             refresh_token_hash  CHAR(64)        NOT NULL COMMENT '리프레시 토큰 해시',
                             expires_at          DATETIME(6)     NOT NULL,
                             revoked_at          DATETIME(6)     NULL,
                             created_at          DATETIME(6)     NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
                             updated_at          DATETIME(6)     NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
                             PRIMARY KEY (session_id),
                             KEY idx_session_user_expires (user_id, expires_at),
                             KEY idx_session_device_expires (device_id, expires_at),
                             CONSTRAINT fk_session_user FOREIGN KEY (user_id) REFERENCES tbl_user (user_id),
                             CONSTRAINT fk_session_device FOREIGN KEY (device_id) REFERENCES tbl_device (device_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='로그인 세션';


CREATE TABLE tbl_encryption_metadata (
                                         enc_id               BIGINT UNSIGNED AUTO_INCREMENT,
                                         entity_type          VARCHAR(30)     NOT NULL COMMENT '예: tbl_user',
                                         entity_id            BINARY(16)      NOT NULL COMMENT '대상 PK',
                                         column_name          VARCHAR(64)     NOT NULL COMMENT '암호화 컬럼',
                                         key_version          VARCHAR(32)     NOT NULL COMMENT 'KMS 키 버전',
                                         algorithm            VARCHAR(32)     NOT NULL DEFAULT 'AES-256-GCM',
                                         encrypted_at         DATETIME(6)     NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
                                         created_at           DATETIME(6)     NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
                                         PRIMARY KEY (enc_id),
                                         KEY idx_enc_entity (entity_type, entity_id, column_name, key_version)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='암호화 메타데이터';
...
CREATE TABLE tbl_transfer_failure (
                                      failure_id           BIGINT UNSIGNED AUTO_INCREMENT,
                                      order_id             BINARY(16)      NOT NULL,
                                      ...
                                      KEY idx_failure_order_time (order_id, failed_at),
                                      CONSTRAINT fk_failure_order FOREIGN KEY (order_id) REFERENCES tbl_transfer_order (order_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='이체 실패 상세';
...
CREATE TABLE tbl_outbox_event (
                                  outbox_id            BIGINT UNSIGNED AUTO_INCREMENT,
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='이벤트 발행 아웃박스';
...
CREATE TABLE tbl_fraud_detection_log (
                                         detection_id         BIGINT UNSIGNED AUTO_INCREMENT,
                                         rule_id              VARCHAR(32)     NOT NULL,
                                         ...
                                         KEY idx_fraud_user_time (user_id, detected_at),
                                         KEY idx_fraud_order_time (order_id, detected_at),
                                         CONSTRAINT fk_fraud_log_rule FOREIGN KEY (rule_id) REFERENCES tbl_fraud_rule (rule_id),
                                         CONSTRAINT fk_fraud_log_order FOREIGN KEY (order_id) REFERENCES tbl_transfer_order (order_id),
                                         CONSTRAINT fk_fraud_log_user FOREIGN KEY (user_id) REFERENCES tbl_user (user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='이상거래 탐지 로그';
