...
BERTY는 **사회초년생(20·30대 청년)** 을 핵심 페르소나로 한 **금융 에이전트** 서비스입니다.

> "월말 적자 반복형 사회초년생"이 마이데이터·오픈뱅킹으로 자산을 연결하면, BERTY가 **30일 현금흐름**을 예측하고 **위험일을 미리 알리며**, **쉬운 말로 행동을 추천**하고, **청년 정책**까지 매칭해 줍니다. 가족 동의 시 **이상 거래는 가족에게 실시간**으로 알립니다.

### 0.1 한눈에 보는 가치

| 사용자가 얻는 것 | BERTY가 하는 일 |
| --- | --- |
...
| 2. 생체·디바이스 | WebAuthn 등록·인증, 신뢰 디바이스 관리 | `BertyController`(security/*), `DeviceManagementController` |
| 3. 동의·연결관리 | 마이데이터/소셜/생체 연결 해제, 동의 이력 | `ConsentManagementController`, `MyDataInstitutionController` |
| 4. 자산·마이데이터 | 자산 요약/추이, 마이데이터 OAuth | `BertyController`(assets/*, mydata/*) |
| 5. 거래·카테고리 | 거래 동기화, 자동 카테고리, 재분류 | `TransactionController` |
| 6. 현금흐름 예측 | 30일 예측, 위험진단, 캘린더, 위험원인, 기준 입력 | `BertyController`(cashflow/*), `CashflowManagementController` |
| 7. 행동추천 | Top-1 행동, 실행, 피드백, 추적 | `BertyController`(action/*), `ActionTrackingController`, `UserFeedbackController` |
| 8. 정책 매칭 | 청년 정책 추천, 신청 표시, 정책 관리(관리자) | `BertyController`(policy/*), `PolicyAdminController` |
| 9. 페르소나 | 직업/거주/소득 추론·수정 | `PersonaController` |
| 10. 송금 | 내부 이체, 5개 외부은행, 오픈뱅킹 이체, 등록 계좌 | `BertyController`(transfers/*, external/*), `RegisteredAccountController` |
| 11. 가족 보호 | 가족 동의, 가족 알림(SSE), 가족 대시보드 | `BertyController`(family/*) |
| 12. 알림 | 위험일/결제일/정책 마감 리마인더 | `NotificationManagementController` |
| 13. 월간 리포트 | 월간 리포트 PDF 생성·발송 | `BertyController`(reports/*), `MonthlyReportBatchService` |
| 14. AI·음성·상담 | AI 상담, EasyRead, STT/TTS | `BertyController`(consult/*), `VoiceController` |
...
| 입력 | provider, code(인가코드), redirectUri, state(OAuth CSRF·`berty.social.stub-mode=false`일 때 서버 검증), codeVerifier(PKCE·선택) |
...
| 관련 API | `GET /api/berty/auth/social/{provider}/authorize-url` (응답: authorizeUrl, state)<br>`POST /api/berty/auth/social/{provider}/login`<br>`POST /api/berty/onboarding/profile` (추가 프로필·필수 약관 동의 후 `tbl_consent_record`에 TERMS/PRIVACY 기록)<br>`POST /api/berty/auth/token` (테스트용 JWT — `berty.auth.test-token-enabled=true`이고 `prod` 프로파일이 아닐 때만 허용) |
...
| 관련 API | `POST /api/berty/auth/logout` |
...
| 관련 API | `POST /api/berty/sessions`<br>`POST /api/berty/sessions/refresh`<br>`GET /api/berty/sessions`<br>`DELETE /api/berty/sessions/{id}`<br>`DELETE /api/berty/sessions` (전체) |
...
| 인증 레벨 | LEVEL_2 |
| 관련 API | `POST /api/berty/security/login-risk/evaluate` |
...
| 부수효과 | `tbl_consent_record`에 TERMS, PRIVACY 동의 행 생성(버전은 `berty.onboarding.consent.*-version`) |
| 관련 API | `POST /api/berty/onboarding/profile` |
...
| 적용 범위 | `/api/berty/**` (단, `/api/berty/auth/**`, `/api/berty/admin/**` 제외) |
...
| 관련 API | `/api/berty/security/webauthn/register/begin`<br>`/api/berty/security/webauthn/register/finish`<br>`/api/berty/security/webauthn/authenticate/begin`<br>`/api/berty/security/webauthn/authenticate/finish` |
...
| 관련 API | `GET /api/berty/devices`<br>`PATCH /api/berty/devices/{id}/name`<br>`DELETE /api/berty/devices/{id}` |
...
| 관련 API | `POST /api/berty/security/level2/proof` |
...
| 관련 API | `GET /api/berty/consents`<br>`POST /api/berty/consents/{id}/revoke` |
...
| 관련 API | `GET /api/berty/mydata/institutions`<br>`GET /api/berty/mydata/institutions/{code}/authorize`<br>`POST /api/berty/mydata/institutions/{code}/callback`<br>`DELETE /api/berty/mydata/institutions/{code}` |
...
| 인증 레벨 | LEVEL_3 |
| 관련 API | `DELETE /api/berty/consents/social/{provider}`<br>`DELETE /api/berty/consents/biometric` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/assets/summary`<br>`GET /api/berty/assets/trend` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `POST /api/berty/transactions/sync` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/transactions?from=&to=` |
...
| 인증 레벨 | LEVEL_2 |
| 관련 API | `POST /api/berty/transactions/recategorize` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/cashflow/forecast` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/cashflow/risk` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/cashflow-management/calendar` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/cashflow-management/risk-causes` |
...
| 관련 API | `POST /api/berty/cashflow/criteria`<br>`GET /api/berty/cashflow/criteria` |
...
| 관련 API | `GET/POST/DELETE /api/berty/cashflow-management/schedules`<br>`GET /api/berty/cashflow/recurring-expenses` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/cashflow/action` |
...
| 인증 레벨 | LEVEL_2 |
| 관련 API | `POST /api/berty/cashflow/action/execute` |
...
| 관련 API | `POST /api/berty/cashflow/action/feedback`<br>`GET /api/berty/cashflow/action/feedback-summary`<br>`GET /api/berty/actions/tracking/{actionType}` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `POST /api/berty/feedback` |
...
| 관련 API | `GET /api/berty/policy/match`<br>`POST /api/berty/policy/{policyCode}/apply` |
...
| 인증 레벨 | LEVEL_3 |
| 관련 API | `GET/POST/DELETE /api/berty/admin/policies` |
...
| 관련 API | `GET /api/berty/persona/{userId}`<br>`PUT /api/berty/persona/{userId}`<br>`POST /api/berty/persona/{userId}/reinfer` |
...
| 인증 레벨 | LEVEL_3 (생체) |
| 관련 API | `POST /api/berty/transfers`<br>`GET /api/berty/transfers/{id}`<br>`GET /api/berty/transfers?userId=` |
...
| 인증 레벨 | LEVEL_3 |
| 관련 API | `POST /api/berty/external/{kakao-bank|hana-bank|kb-bank|shinhan-bank|im-bank}/transfer`<br>`POST /api/berty/external/openbanking/transfer` |
| 부가 조회 | `GET /api/berty/external/openbanking/{accounts|balance|transactions}`<br>`GET /api/berty/external/pension/summary` |
...
| 관련 API | `POST /api/berty/registered-accounts`<br>`GET /api/berty/registered-accounts`<br>`GET /api/berty/registered-accounts/decrypted` (LEVEL_3)<br>`DELETE /api/berty/registered-accounts` |
...
| 관련 API | `POST/PATCH/DELETE/GET /api/berty/family/consents` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/family-alerts?userId=`<br>`GET /api/berty/family-alerts/stream?userId=` (SSE) |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `GET /api/berty/family/dashboard?userId=` |
...
| 관련 API | `GET /api/berty/notifications?userId=`<br>`POST /api/berty/notifications/generate-reminders` |
...
| 자동화 | `MonthlyReportBatchService` 매월 1일 09시 cron으로 전체 사용자 발송 (`berty.report.cron` 설정) |
...
| 인증 레벨 | 조회 LEVEL_1 |
| 관련 API | `GET /api/berty/reports/monthly?userId=` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `POST /api/berty/ai/consult` |
...
| 인증 레벨 | LEVEL_1 |
| 관련 API | `POST /api/berty/voice/stt`, `POST /api/berty/voice/tts` |
...
| 인증 레벨 | LEVEL_3 |
| 관련 API | `GET /api/berty/audit-logs?size=` |
...
| 관련 API | `GET /api/berty/kpi/user/{userId}`<br>`GET /api/berty/kpi/global` |
...
| 인증 레벨 | LEVEL_3 |
| 관련 API | `/api/berty/admin/ai-templates` |
...
| 인증 레벨 | 비로그인 |
| 관련 API | `POST /api/berty/auth/demo/session` |

---

## 3. 데이터 모델 (핵심 테이블)

> 전체 ERD는 `berty-ERD/berty_Table_Ver1.1.sql` 참조. 기획자가 알면 좋은 핵심 테이블만 발췌.
...
| 룰 | 임계값/정책 | 위치 |
| --- | --- | --- |
| 가족 알림 트리거 | 1회 송금 ≥ 1,000,000원 + 가족 동의 ON | `BertyService.FAMILY_ALERT_THRESHOLD` |
| 대형 이체 임계 | 1회 송금 ≥ 3,000,000원 → 이상거래 룰 진입 | `BertyService.LARGE_TRANSFER_THRESHOLD` |
...
| 정책 마감 알림 | 마감까지 0–7일 | 〃 |
| 월간 리포트 발송 | 매월 1일 09:00 | `MonthlyReportBatchService` (`berty.report.cron`) |
...
| 카테고리 | 메서드 | 경로 |
| --- | --- | --- |
| 인증 | POST | `/api/berty/auth/token` (테스트 전용, `berty.auth.test-token-enabled`·`prod` 제한) |
| 인증 | GET | `/api/berty/auth/social/{provider}/authorize-url` (응답: authorizeUrl, state) |
| 인증 | POST | `/api/berty/auth/social/{provider}/login` |
| 인증 | POST | `/api/berty/onboarding/profile` |
| 인증 | POST | `/api/berty/auth/logout` |
| 인증 | POST | `/api/berty/auth/demo/session` |
| 세션 | POST/GET/DELETE | `/api/berty/sessions[/{id}]` |
| 세션 | POST | `/api/berty/sessions/refresh` |
| 보안 | POST | `/api/berty/security/login-risk/evaluate` |
| 보안 | POST | `/api/berty/security/level2/proof` |
| 생체 | POST | `/api/berty/security/webauthn/{register|authenticate}/{begin|finish}` |
| 디바이스 | GET/PATCH/DELETE | `/api/berty/devices[/{id}[/name]]` |
| 동의 | GET/POST | `/api/berty/consents[/{id}/revoke]` |
| 동의 | DELETE | `/api/berty/consents/{mydata|social|biometric}/...` |
| 마이데이터 | GET/POST/DELETE | `/api/berty/mydata/institutions/...` |
| 자산 | GET | `/api/berty/assets/{summary|trend}` |
| 거래 | GET/POST | `/api/berty/transactions[/sync|/recategorize]` |
| 현금흐름 | GET | `/api/berty/cashflow/{forecast|risk|action|recurring-expenses|criteria}` |
| 현금흐름 | POST | `/api/berty/cashflow/{criteria|action/execute|action/feedback}` |
| 현금흐름 | GET | `/api/berty/cashflow/action/feedback-summary` |
| 캘린더 | GET/POST/DELETE | `/api/berty/cashflow-management/{calendar|schedules|risk-causes}` |
| 추천 추적 | GET | `/api/berty/actions/tracking/{actionType}` |
| 피드백 | POST | `/api/berty/feedback` |
| 정책 | GET/POST | `/api/berty/policy/{match|{code}/apply}` |
| 정책(관리) | GET/POST/DELETE | `/api/berty/admin/policies[/{code}]` |
| 페르소나 | GET/PUT/POST | `/api/berty/persona/{userId}[/reinfer]` |
| 송금 | POST/GET | `/api/berty/transfers[/{id}]` |
| 외부은행 | POST/GET | `/api/berty/external/{kakao-bank|hana-bank|kb-bank|shinhan-bank|im-bank|openbanking|pension}/...` |
| 등록계좌 | POST/GET/DELETE | `/api/berty/registered-accounts[/decrypted]` |
| 가족 | POST/PATCH/DELETE/GET | `/api/berty/family/consents` |
| 가족 | GET | `/api/berty/family-alerts[/stream]` |
| 가족 | GET | `/api/berty/family/dashboard` |
| 알림 | GET/POST | `/api/berty/notifications[/generate-reminders]` |
| 리포트 | GET | `/api/berty/reports/monthly` |
| AI/상담 | POST | `/api/berty/{consult|ai/consult}` |
| 음성 | POST | `/api/berty/voice/{stt|tts}` |
| 운영 | GET | `/api/berty/audit-logs` |
| 운영 | GET | `/api/berty/kpi/{user/{id}|global}` |
| 운영 | GET/POST/DELETE | `/api/berty/codes/...` |
| 운영 | GET/POST/DELETE | `/api/berty/admin/ai-templates[/{key}]` |
...
> 본 문서는 현재 소스 트리(`src/main/java/com/berty/**`)와 ERD(`berty-ERD/berty_Table_Ver1.1.sql`)를 기준으로 작성되었습니다. 컨트롤러 추가/변경 시 본 문서의 §2 / §8을 함께 갱신해 주세요.