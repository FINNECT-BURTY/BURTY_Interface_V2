 ...
 *
 * <pre>
 * <b>Description  : [테스트] 공통 통합 테스트 (BurtyE2ETests)</b>
...
class BurtyE2ETests extends IntegrationTestBase {
  ...
  void socialLoginIssuesBurtyTokenAndPersistsAccount() {
    ...
    ResponseEntity<String> login =
        restTemplate.postForEntity(
            ...
            new HttpEntity<>(
                "{\"code\":\"test-social-code\",\"state\":\"state-1\"}", jsonHeaders()),
            String.class);
  ...
  void googleSocialLoginIssuesBurtyToken() {
    ...
    ResponseEntity<String> login =
        restTemplate.postForEntity(
            ...
            new HttpEntity<>(
                "{\"code\":\"test-google-code\",\"state\":\"state-g\"}", jsonHeaders()),
            String.class);
  ...
  void profileOnboardingAfterSocialLogin() {
    ...
    String phone =
        "010"
            + String.format(
                "%08d", Math.abs(UUID.randomUUID().getLeastSignificantBits()) % 100_000_000);
    String onboardJson =
        "{"
            + "\"phone\":\""
            + phone
    ...
    Assertions.assertTrue(
        consentRecordRepository.findByUser_UserId(Long.parseLong(socialUserId)).size() >= 2);
    ...
    ResponseEntity<String> onboard2 =
        restTemplate.postForEntity(
            base + "/api/v1/onboarding/profile",
            new HttpEntity<>(onboardJson, authHeaders),
            String.class);
  ...
  void authToConsultToFamilyAlertFlow() throws Exception {
    ...
    ResponseEntity<String> consult =
        restTemplate.postForEntity(
            base + "/api/v1/consult",
            new HttpEntity<>(
                "{\"userId\":\"" + userId + "\",\"question\":\"이번달 괜찮아?\"}", authHeaders),
            String.class);
