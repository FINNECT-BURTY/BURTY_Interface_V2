 ...
 *
 * <p>애플리케이션의 Flyway 설정과 {@code ddl-auto=validate} 를 그대로 쓴다. 별도 스크립트로 스키마를 만들면 실제 애플리케이션이 기대하는 것과
 * 어긋날 수 있고, 그러면 복구 검증이 의미를 잃는다.
 *
...
class CiSchemaTasks {
  ...
  static void datasource(DynamicPropertyRegistry registry) {
    ...
    registry.add(
        "spring.datasource.password", () -> System.getProperty("burty.ci.db.password", ""));
  ...
  private static String required(String key) {
    ...
    if (value == null || value.isBlank()) {
      throw new IllegalStateException("시스템 프로퍼티 " + key + " 가 필요합니다 (CI 전용 작업입니다)");
    }
    return value;
  }
