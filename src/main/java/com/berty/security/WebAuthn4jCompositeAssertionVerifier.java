...
public class WebAuthn4jCompositeAssertionVerifier implements WebAuthnAssertionVerifier {
    ...
    public VerificationResult verifyRegistration(String payload, String expectedChallenge, String expectedOrigin, String expectedRpId, long currentSignCount) {
        try {
            ServerProperty serverProperty = serverProperty(expectedChallenge, expectedOrigin, expectedRpId);
            RegistrationData data = registrationManager.verify(payload, new RegistrationParameters(serverProperty, false));
            var attested = data.getAttestationObject().getAuthenticatorData().getAttestedCredentialData();
            if (attested == null) {
                return fallback.verifyRegistration(payload, expectedChallenge, expectedOrigin, expectedRpId, currentSignCount);
            }
    ...
    public VerificationResult verifyAuthentication(String payload, String expectedChallenge, String expectedOrigin, String expectedRpId, long currentSignCount,
        ...
        if (storedCredential == null || storedCredential.getCosePublicKey().length == 0) {
            return fallback.verifyAuthentication(payload, expectedChallenge, expectedOrigin, expectedRpId, currentSignCount, storedCredential);
        }
        COSEKey coseKey;
        try {
            coseKey = objectConverter.getCborConverter().readValue(storedCredential.getCosePublicKey(), COSEKey.class);
        } catch (Exception ignored) {
            return fallback.verifyAuthentication(payload, expectedChallenge, expectedOrigin, expectedRpId, currentSignCount, storedCredential);
        }
        try {
            ServerProperty serverProperty = serverProperty(expectedChallenge, expectedOrigin, expectedRpId);
            AttestedCredentialData acd = new AttestedCredentialData(AAGUID.ZERO, storedCredential.getCredentialIdRaw(), coseKey);
            AuthenticatorImpl authenticator = new AuthenticatorImpl(acd, new NoneAttestationStatement(), storedCredential.getSignCount());
            AuthenticationParameters parameters = new AuthenticationParameters(serverProperty, authenticator, false);
            ...
            String keyB64 = Base64.getUrlEncoder().withoutPadding().encodeToString(storedCredential.getCosePublicKey());
            return new VerificationResult(true, next, credB64, keyB64);
        } catch (Exception ignored) {
            return fallback.verifyAuthentication(payload, expectedChallenge, expectedOrigin, expectedRpId, currentSignCount, storedCredential);
        }
    ...
    private ServerProperty serverProperty(String expectedChallenge, String expectedOrigin, String expectedRpId) {
