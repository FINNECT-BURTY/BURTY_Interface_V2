...
public class MonthlyReportBatchService {
    ...
    private final BertyUseCase bertyUseCase;
    private final MonthlyReportDeliveryPort deliveryPort;
    private final MonthlyReportHistoryPort historyPort;
    private final UserRepository userRepository;

    public MonthlyReportBatchService(BertyUseCase bertyUseCase, MonthlyReportDeliveryPort deliveryPort,
                                     MonthlyReportHistoryPort historyPort, UserRepository userRepository) {
        this.bertyUseCase = bertyUseCase;
        this.deliveryPort = deliveryPort;
        this.historyPort = historyPort;
        this.userRepository = userRepository;
    }

    @Scheduled(cron = "${berty.report.cron:0 0 9 1 * *}")
    public void generateAndDeliverMonthlyReports() {
        ...
        for (String userId : userIds) {
            try {
                MonthlyReport report = bertyUseCase.createMonthlyReport(userId);
    ...
    private byte[] createPdf(MonthlyReport report) throws Exception {
        ...
        document.open();
        document.add(new Paragraph("BERTY Monthly Report"));
