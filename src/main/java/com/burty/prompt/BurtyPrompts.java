 ...
 *
 * <pre>
 * <b>Description  : 공통 (BurtyPrompts)</b>
 * <b>Project Name : BURTY</b>
 * package  : com.burty.prompt
 * </pre>
 *
 * @author : RosieOh
 * @version : 1.0
 ...
 * BURTY LLM 프롬프트 카탈로그.
 *
 * <p>시스템 역할 프롬프트(상수)와 컨텍스트 주입형 사용자 프롬프트 빌더(메서드)를 한 곳에 모은다. 새 use-case가 추가되면 여기에 상수 + 빌더 한 쌍을 추가한다.
 */
public final class BurtyPrompts {

  private BurtyPrompts() {}
  ...
  public static String advisoryUserPrompt(
      String question, PersonaProfileEntity persona, AssetSnapshot snapshot, RiskAssessment risk) {
    return """
                사용자 질문: %s
                페르소나:
  ...
  public static String monthlyReportUserPrompt(
      String period,
    ...
    return """
        ...
        .formatted(
            ...
            riskDate(risk),
            riskReason(risk),
            nullSafe(topActionTitle),
            estimatedImprovement);
  }

  /** 행동 추천 설명용 user prompt. */
  public static String recommendationExplainerUserPrompt(
  ...
  private static String riskLevel(RiskAssessment r) {
    return r == null ? "-" : nullSafe(r.level());
  }

  private static long projectedBalance(RiskAssessment r) {
    return r == null ? 0L : r.projectedBalance();
  }

  private static String riskDate(RiskAssessment r) {
    if (r == null || r.riskDate() == null) return "없음";
    return r.riskDate().toString();
  }

  private static String riskReason(RiskAssessment r) {
    return r == null ? "-" : nullSafe(r.reason());
  }
}
