 ...
 *   2) Set burty.kms.provider=AWS_KMS
 *   3) Configure AWS credentials (env / instance profile / SSO)
 *   4) Map aliases to KMS key IDs via burty.kms.aws.alias-map.* properties:
 *        burty.kms.aws.alias-map.burty.account-no=arn:aws:kms:ap-northeast-2:...:key/...
 *
...
@Component
@Primary
@ConditionalOnProperty(prefix = "burty.kms", name = "provider", havingValue = "AWS_KMS")
public class AwsKmsAdapter implements KmsPort {
    ...
    public AwsKmsAdapter(@Value("${burty.kms.aws.region:ap-northeast-2}") String defaultRegion) {
