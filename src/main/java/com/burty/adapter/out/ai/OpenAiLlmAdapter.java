 ...
 *
 * <pre>
 * <b>Description  : 외부연동 외부 연동 어댑터 (OpenAiLlmAdapter)</b>
...
public class OpenAiLlmAdapter implements LlmPort {
  ...
  private String contextualFallback(String userPrompt) {
    ...
    return "이번 달 흐름은 안정적이에요. 큰 변동은 없을 전망이에요. " + "이번 주 안에 자동이체 내역 한 번 점검해주세요.";
  }
  ...
  private record FallbackContext(
    ...
    boolean red() {
      return "RED".equals(level) || raw.contains("level\":\"RED");
    }

    boolean yellow() {
      return "YELLOW".equals(level) || raw.contains("level\":\"YELLOW");
    }

    boolean freelancer() {
      return raw.contains("FREELANCER");
    }

    boolean jobSeeker() {
      return raw.contains("JOB_SEEKER");
    }

    boolean rentRelated() {
      return containsAny(reason, "월세", "관리비", "고정지출", "고정비");
    }

    boolean cardRelated() {
      return containsAny(reason, "카드", "자동결제", "결제일");
    }

    boolean loanRelated() {
      return containsAny(reason, "대출", "상환", "이자");
    }

    boolean highSpend() {
      return monthlySpend >= 2_500_000L;
    }
