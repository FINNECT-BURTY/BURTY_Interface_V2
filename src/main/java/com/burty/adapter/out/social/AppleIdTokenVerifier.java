 ...
 *
 * <pre>
 * <b>Description  : 소셜로그인 (AppleIdTokenVerifier)</b>
...
public class AppleIdTokenVerifier {
  ...
  private PublicKey resolvePublicKey(String kid) throws Exception {
    ...
    if (refreshed == null) {
      throw new BusinessException(
          ErrorCode.EXTERNAL_API_ERROR, "Apple JWKS 에서 kid=" + kid + " 키를 찾을 수 없습니다.");
    }
  ...
  private void refreshJwksIfStale() {
    if (jwksCache != null
        && System.currentTimeMillis() - jwksFetchedAtMs < Duration.ofHours(6).toMillis()) {
      return;
    }
    refreshJwksForce();
  }
  ...
  private void refreshJwksForce() {
    String body = webClient.get().uri(JWKS_URL).retrieve().bodyToMono(String.class).block(timeout);
    try {
      ...
      throw new BusinessException(
          ErrorCode.EXTERNAL_API_ERROR, SocialProvider.APPLE + " JWKS 조회에 실패했습니다.");
    }
  }
