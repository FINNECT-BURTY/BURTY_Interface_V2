 ...
 *
 * <pre>
 * <b>Description  : 소셜로그인 (AppleClientSecretGenerator)</b>
...
/**
 * Apple Sign in token endpoint 용 client_secret (ES256 JWT) 생성. {@link
 * SocialLoginProperties.Provider#getClientSecret()} 정적 값 대신 사용.
 */
@Component
public class AppleClientSecretGenerator {
  ...
  public String generate(SocialLoginProperties.Provider config) {
    if (isBlank(config.getTeamId())
        || isBlank(config.getKeyId())
    ...
    try {
      ...
      throw new BusinessException(
          ErrorCode.EXTERNAL_API_ERROR, SocialProvider.APPLE + " client_secret JWT 생성에 실패했습니다.");
    }
  }
