...
import com.burty.application.port.out.ShinhanBankPort;
...
@Component
public class ShinhanBankApiAdapter implements ShinhanBankPort {
    ...
    public Map<String, Object> transfer(String userId, String toAccount, long amount) {
        if (!properties.isStubMode()) {
            ...
            headers.add("X-API-KEY", properties.getShinhanApiKey());
            Map<String, Object> body = Map.of("userId", userId, "toAccount", toAccount, "amount", amount);
            Map response = restTemplate.postForObject(properties.getShinhanTransferUrl(), new HttpEntity<>(body, headers), Map.class);
