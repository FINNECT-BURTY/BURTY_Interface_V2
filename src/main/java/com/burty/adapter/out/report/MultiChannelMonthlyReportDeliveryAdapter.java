 ...
 *
 * <pre>
 * <b>Description  : 외부연동 외부 연동 어댑터 (MultiChannelMonthlyReportDeliveryAdapter)</b>
...
public class MultiChannelMonthlyReportDeliveryAdapter implements MonthlyReportDeliveryPort {
  ...
  public MultiChannelMonthlyReportDeliveryAdapter(
      NotificationRepository notificationRepository,
      UserRepository userRepository,
      MonthlyReportDeliveryProperties deliveryProperties,
  ...
  public void deliver(String userId, byte[] pdfBytes, String fileName) {
    log.info(LogMessages.Report.MONTHLY_DELIVERY, userId, fileName, pdfBytes.length);
    ...
    if (deliveryProperties.isWebhookEnabled()
        && deliveryProperties.getWebhookUrl() != null
        && !deliveryProperties.getWebhookUrl().isBlank()) {
  ...
  private void postWebhook(String userId, String fileName, byte[] pdfBytes) {
    try {
      ...
      restTemplate.postForEntity(
          deliveryProperties.getWebhookUrl(), new HttpEntity<>(body, headers), String.class);
