...
@Tag(name = "BURTY Auth - Google", description = "구글 OAuth 로그인 (authorize-url / SPA login / BFF callback)")
public class GoogleAuthController extends BaseController {
    ...
    public GoogleAuthController(SocialLoginUseCase socialLoginUseCase, SocialAuthSupport support) {
    ...
    @Operation(
            summary = "구글 인가 URL 생성",
            description = "구글 OAuth 인가 URL과 state를 반환합니다. `redirectUri` 를 넘기면 구글에 전달 — Google Cloud Console 에 사전 등록 필수 " +
                    "(아니면 redirect_uri_mismatch). default 는 `burty.social.google.redirect-uri`.",
            security = {}
    )
    ...
    @PostMapping("/login")
    @Operation(
            summary = "구글 로그인 (SPA / API client 용)",
            description = "구글 authorization code를 검증하고 BURTY access + refresh token 쌍을 응답 body 로 반환합니다.",
            security = {}
    )
    ...
    @Operation(
            summary = "구글 BFF 콜백",
            description = "구글이 redirect 하는 GET 엔드포인트. 토큰 교환 후 access/refresh 를 HttpOnly 쿠키로 set + FE 페이지로 302.",
            security = {}
    )
