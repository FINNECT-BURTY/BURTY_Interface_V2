 ...
 *
 * <pre>
 * <b>Description  : 설정 설정 (SwaggerConfig)</b>
...
public class SwaggerConfig {
  ...
  public OpenAPI customOpenAPI() {
    ...
    // Info 메타데이터
    Info info = new Info().title("BURTY API").description("BURTY REST API 문서").version("1.0.0");

    // SecurityRequirement - 모든 API에 기본 적용 -> 보안 규칙 설정
    SecurityRequirement securityRequirement = new SecurityRequirement().addList(BEARER_SCHEME);

    OpenAPI openAPI =
        new OpenAPI().info(info).components(components).addSecurityItem(securityRequirement);
