 ...
 *
 * <pre>
 * <b>Description  : 코어 핸들러 (GlobalExceptionHandler)</b>
 * <b>Project Name : BURTY</b>
 * package  : com.burty.core.handler
 * </pre>
 *
 * @author : RosieOh
 * @version : 1.0
...
import com.burty.core.error.dto.ErrorResponse;
import com.burty.core.error.enums.ErrorCode;
import com.burty.core.error.enums.ErrorCodeHttpStatus;
import com.burty.core.exception.BusinessException;
import com.burty.core.exception.ResourceNotFoundException;
import com.burty.core.jenkins.exception.JenkinsBuildException;
import com.burty.core.jenkins.exception.JenkinsConnectionException;
import com.burty.core.jenkins.exception.JenkinsException;
...
public class GlobalExceptionHandler {
  ...
  public GlobalExceptionHandler(LoginFailLogUtil loginFailLogUtil) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleBusinessException(
    ...
    if (e.getErrorCode() == ErrorCode.UNAUTHORIZED && isAuthRelatedPath(request)) {
      loginFailLogUtil.logAuthFailure(request, e.getMessage());
    }

    ErrorResponse errorResponse = ErrorResponse.of(e.getErrorCode(), request.getRequestURI());

    return ResponseEntity.status(ErrorCodeHttpStatus.resolve(e.getErrorCode()))
        .body(
            ApiResponse.error(errorResponse.getMessage(), String.valueOf(errorResponse.getCode())));
  }

  /** 커스텀 인증 예외 처리 */
  @ExceptionHandler(com.burty.core.exception.AuthenticationException.class)
  public ResponseEntity<ApiResponse<ErrorResponse>> handleAuthenticationException(
      com.burty.core.exception.AuthenticationException e, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleResourceNotFoundException(
    ...
    ErrorResponse errorResponse =
        ErrorResponse.of(ErrorCode.ENTITY_NOT_FOUND, request.getRequestURI(), e.getMessage());
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleSpringAuthenticationException(
    ...
    if (isAuthRelatedPath(request)) {
      loginFailLogUtil.logAuthFailure(request, e.getMessage());
    }

    ErrorResponse errorResponse =
        ErrorResponse.of(ErrorCode.INVALID_CREDENTIALS, request.getRequestURI());
  ...
  private static boolean isAuthRelatedPath(HttpServletRequest request) {
    String uri = request.getRequestURI();
    return uri != null
        && (uri.contains("/auth")
            || uri.contains("/admin/auth")
            || uri.contains("/sessions")
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleValidationException(
    ...
    ErrorResponse errorResponse =
        ErrorResponse.of(ErrorCode.INVALID_INPUT_VALUE, request.getRequestURI(), errorMessage);
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleHttpRequestMethodNotSupportedException(
    ...
    ErrorResponse errorResponse =
        ErrorResponse.of(ErrorCode.METHOD_NOT_ALLOWED, request.getRequestURI());
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleJenkinsConnectionException(
      JenkinsConnectionException e, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleJenkinsJobNotFoundException(
      JenkinsJobNotFoundException e, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleJenkinsBuildException(
      JenkinsBuildException e, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleJenkinsException(
      JenkinsException e, HttpServletRequest request) {
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleNullPointerException(
    ...
    log.error("NullPointerException: URI={}", request.getRequestURI(), e);
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleIndexOutOfBoundsException(
      IndexOutOfBoundsException e, HttpServletRequest request) {
    log.error("IndexOutOfBoundsException: URI={}", request.getRequestURI(), e);
  ...
  public ResponseEntity<ApiResponse<ErrorResponse>> handleException(
    ...
    // 클라이언트가 연결을 끊은 경우 (동영상 스트리밍 range request 중 정상 종료) - Spring에 위임
