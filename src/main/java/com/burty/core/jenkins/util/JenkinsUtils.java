 ...
 *
 * <pre>
 * <b>Description  : 코어 (JenkinsUtils)</b>
 * <b>Project Name : BURTY</b>
 * package  : com.burty.core.jenkins.util
 * </pre>
 *
 * @author : RosieOh
 * @version : 1.0
...
import com.burty.core.jenkins.dto.BuildResponse;
import com.burty.core.jenkins.dto.JobCreateRequest;
import com.burty.core.jenkins.dto.JobResponse;
import com.burty.core.jenkins.dto.NodeResponse;
...
public class JenkinsUtils {
  ...
  public static JobResponse convertJsonToJobResponse(JsonNode json) {
    ...
    if (json.has("builds")) {
      ...
      builds =
          StreamSupport.stream(buildsNode.spliterator(), false)
    ...
    return JobResponse.builder()
        ...
        .lastBuildNumber(
            json.has("lastBuild") && json.get("lastBuild").has("number")
                ? json.get("lastBuild").get("number").asInt()
                : null)
        .lastSuccessfulBuildNumber(
            json.has("lastSuccessfulBuild") && json.get("lastSuccessfulBuild").has("number")
                ? json.get("lastSuccessfulBuild").get("number").asInt()
                : null)
        .lastFailedBuildNumber(
            json.has("lastFailedBuild") && json.get("lastFailedBuild").has("number")
                ? json.get("lastFailedBuild").get("number").asInt()
                : null)
  ...
  public static BuildResponse convertJsonToBuildResponse(JsonNode json) {
        ...
        if (action.has("parameters")) {
          ...
          for (JsonNode param : params) {
            if (param.has("name") && param.has("value")) {
              parameters.put(param.get("name").asText(), param.get("value").asText());
            }
          }
        }
      }
    ...
    return BuildResponse.builder()
        ...
        .url(json.has("url") ? json.get("url").asText() : null)
        .result(
            json.has("result") && !json.get("result").isNull() ? json.get("result").asText() : null)
        .building(json.has("building") ? json.get("building").asBoolean() : null)
        .duration(json.has("duration") ? json.get("duration").asLong() : null)
        .estimatedDuration(
            json.has("estimatedDuration") ? json.get("estimatedDuration").asLong() : null)
  ...
  public static NodeResponse convertJsonToNodeResponse(JsonNode json) {
    return NodeResponse.builder()
        ...
        .temporarilyOffline(
            json.has("temporarilyOffline") ? json.get("temporarilyOffline").asBoolean() : null)
        .numExecutors(json.has("numExecutors") ? json.get("numExecutors").asInt() : null)
        .idle(json.has("idle") ? json.get("idle").asBoolean() : null)
        .offlineCauseReason(
            json.has("offlineCauseReason") ? json.get("offlineCauseReason").asText() : null)
        .build();
  }
  ...
  public static String generateJobXml(JobCreateRequest request) {
    ...
    if (request.getGitUrl() != null) {
      ...
      xml.append("        <name>")
          .append(request.getGitBranch() != null ? request.getGitBranch() : "*/main")
          .append("</name>\n");
      xml.append("      </hudson.plugins.git.BranchSpec>\n");
      xml.append("    </branches>\n");
      xml.append("  </scm>\n");
    ...
    if (request.getBuildScript() != null) {
      ...
      xml.append("      <command>")
          .append(escapeXml(request.getBuildScript()))
          .append("</command>\n");
      xml.append("    </hudson.tasks.Shell>\n");
      xml.append("  </builders>\n");
    } else {
