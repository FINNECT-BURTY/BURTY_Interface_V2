 ...
 *
 * <pre>
 * <b>Description  : 배치 배치 작업 (RecurringExpenseLearnBatch)</b>
...
/**
 * 월 1회 거래내역을 분석해 같은 카테고리에서 3회 이상, 표준편차 작은 지출을 RecurringExpenseEntity로 학습. 이미 등록된 항목은
 * occurrenceCount/avgAmount/dayOfMonth/lastSeenAt만 갱신.
 */
@Component
public class RecurringExpenseLearnBatch {
  ...
  public RecurringExpenseLearnBatch(
  ...
  @SchedulerLock(
      name = "RecurringExpenseLearnBatch",
  ...
  public void learnMonthly() {
    ...
    log.info(LogMessages.Batch.RECURRING_EXPENSE_LEARN, totalUsers, totalLearned, since);
    auditLogger.logSuccess(
        "system",
        "RECURRING_LEARN_BATCH",
        "BATCH",
  ...
  private int learnForUser(Long userId, LocalDate since) {
    List<TransactionEntity> txs =
        transactionRepository.findByUserIdAndTxnDateBetweenOrderByTxnDateDesc(
            userId, since, LocalDate.now());
  ...
  private static final class Aggregator {
    ...
    void add(TransactionEntity tx) {
      ...
      String label =
          tx.getMerchant() != null && !tx.getMerchant().isBlank()
              ? tx.getMerchant()
