...
import com.nuri.config.JwtProperties;
...
public class RiskProofService {
    ...
    public String issue(String userId, RiskLevel level) {
        ...
        return Base64.getUrlEncoder().withoutPadding().encodeToString(tokenRaw.getBytes(StandardCharsets.UTF_8));
    }
    ...
    public boolean verify(String token, String expectedUserId, RiskLevel requiredLevel) {
        ...
        try {
            ...
            String expectedSig = sign("%s|%s|%d|%s".formatted(userId, issuedLevel.name(), expiresAt, nonce));
